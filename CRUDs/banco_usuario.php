...
function buscaUsuario($conexao, $login, $senha){
	$query = "select * from usuario where nm_usuario = '{$login}' and "
...
function nroUsuario($conexao, $login, $senha){
	$query = "select * from usuario where nm_usuario = '{$login}' "
        . "and cd_senha_usuario = '{$senha}'";
	$resultado = mysqli_query($conexao, $query);
...
function insereUsuario($conexao, $nome, $cpf, $senha, $nivel){
    $query = "insert into usuario (cpf_usuario, nm_usuario, cd_senha_usuario, "
            . "cd_tipo_usuario) VALUES({$cpf},'{$nome}','{$senha}',{$nivel})";
...
function alterarUsuario($conexao,$id,$nome,$cpf,$senha, $nivel){
        $query = "update usuario set nm_usuario='{$nome}',"
        . "cpf_usuario='{$cpf}',cd_senha_usuario='{$senha}',"
        . "cd_tipo_usuario={$nivel} where cd_usuario = {$id}";
	return mysqli_query($conexao, $query);
}
